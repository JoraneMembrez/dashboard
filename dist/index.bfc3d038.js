...
const getCoordinates = ()=>{
    return new Promise((res, rej)=>// getCurrentPosition --> permet d'avoir la localisation géogrpahique actuelle du périphérique
...
const getDashboardInformation = ()=>{
    // Cherche la position
    getPosition()// Puis fait une requête paralelle vers les API en fonction des géodonnées
    .then((res)=>{
    ...
    .then((data)=>{
        ...
        if (station) return fetch(`https://transport.opendata.ch/v1/stationboard?station=${station.name}&limit=6`).then((res)=>res.json());
        else throw new Error("No train stations nearby");
